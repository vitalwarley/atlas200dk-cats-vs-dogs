...
File: main.py
Author: vitalwarey
Email: vitalwarley@gmail.com
Github: https://github.com/vitalwarley
Description: this script executes an image classification model into the Atlas 200DK.
Source: https://github.com/Ascend/samples
"""
...
MODEL_PATH = os.path.join(currentPath, "model/model.om")
MODEL_WIDTH = 224
MODEL_HEIGHT = 256
CLS = ["dog", "cat"]
...
class Classify(object):
    ...
    def pre_process(self, image):
        ...
        resized_image = self._dvpp.resize(
            yuv_image, self._model_width, self._model_height
        )
    ...
    def post_process(self, infer_output, image_file):
        ...
        prob = infer_result.flatten()[0]
        idx = 0 if prob > 0.5 else 1

        origin_img = Image.open(image_file)
        draw = ImageDraw.Draw(origin_img)
        font = ImageFont.load_default()
        draw.text((10, 50), CLS[idx], font=font, fill=255)
...
def main():
    ...
    main
    """
    image_dir = os.path.join(currentPath, "samples")
