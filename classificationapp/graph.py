...
class Graph():
    ...
    def create_graph_with_dvpp(self, resize_cfg):
        ...
        with graph.as_default():
            engine = hiai.Engine()
            resize_config = hiai.ResizeConfig(
                resize_width=resize_cfg[0], resize_height=resize_cfg[1])
            nntensorlist_object = engine.resize(
                input_tensor_list=nntensorlist_object, config=resize_config)
            ai_model_desc = hiai.AIModelDescription(
                name=os.path.basename(
                    self.model_path), path=self.model_path)
            ai_config = hiai.AIConfig(
        ...
        if ret != hiai.HiaiPythonStatust.HIAI_PYTHON_OK:
            graph.destroy()
            raise Exception(
                "[create_graph_with_dvpp]: create graph failed, ret ", ret)
